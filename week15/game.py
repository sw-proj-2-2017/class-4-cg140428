...
class HangmanGame(QWidget):
    ...
    def startGame(self):
        self.hangman = Hangman() #Hangman 상태 초기화
        self.guess = Guess(self.word.randFromDB()) #비밀 단어 선택
        self.gameOver = False #게임 진행 여부 초기화

        self.hangmanWindow.setPlaceholderText(self.hangman.currentShape())
        self.currentWord.setText(self.guess.displayCurrent()) #currentWord 내용 표시
        self.guessedChars.setText(self.guess.displayGuessed()) #guessedChars 내용 표시
        self.message.clear() #message내용 표시
